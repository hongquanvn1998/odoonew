...
class Database(http.Controller):
    ...
    #Author: @Thiep Wong
    #Function: Create a database by api
    #Data: type json, method post, id 989999
    #   {
    # "jsonrpc": "2.0"  ,
    # "id":14545454,
    # "params": {
    ...
    #  }
    #  }
    @http.route('/web/database/register',type='json', auth="none") 
    def register(self, **post):
        try:
            if not re.match(DBNAME_PATTERN, post['name']):
                raise Exception(_('Invalid database name. Only alphanumerical characters, underscore, hyphen and dot are allowed.'))
            # country code could be = "False" which is actually True in python
            country_code = post.get('country_code') or False
            dispatch_rpc('db', 'create_database', [post['master_pwd'], post['name'], bool(post.get('demo')), post['lang'], post['password'], post['login'], country_code, post['phone']])
            request.session.authenticate(post['name'], post['login'], post['password'])
